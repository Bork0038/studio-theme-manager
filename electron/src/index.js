...
const isDev = !relativePath.endsWith("resources\\");

process.isDev = isDev;
process.themeFolder = path.join(__dirname, isDev ? "../../themes" : "../../../themes");

const workspacePath = process.themeFolder;
...
websocketManager.on("setTheme", async (connection, data) => {
    ...
    connection.sendBulk([
        { op: "step", data: { steps: 3, totalSteps: 3 }},
        { op: "loaded", data: {} }
    ])
})
